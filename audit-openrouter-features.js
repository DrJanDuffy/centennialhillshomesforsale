...
async function auditOpenRouterFeatures() {
  ...
  try {
    ...
    // Categorize models
    const categories = {
      'Google/Gemini': models.filter(
        (m) => m.id.toLowerCase().includes('google') || m.id.toLowerCase().includes('gemini')
      ),
      'OpenAI/GPT': models.filter(
        (m) => m.id.toLowerCase().includes('openai') || m.id.toLowerCase().includes('gpt')
      ),
      'Anthropic/Claude': models.filter(
        (m) => m.id.toLowerCase().includes('anthropic') || m.id.toLowerCase().includes('claude')
      ),
      'Meta/Llama': models.filter(
        (m) => m.id.toLowerCase().includes('meta') || m.id.toLowerCase().includes('llama')
      ),
      'Image Generation': models.filter(
        (m) =>
    ...
    for (const modelId of testModels) {
      try {
        ...
        if (testResponse.ok) {
          const _testData = await testResponse.json();
          console.log(`  ✅ ${modelId} - Accessible`);
        } else {
          const errorText = await testResponse.text();
          const errorData = JSON.parse(errorText);
          console.log(
            `  ❌ ${modelId} - ${testResponse.status}: ${errorData.error?.message || 'Unknown error'}`
          );
        }
      } catch (error) {
