  ...
  try {
    ...
    if (perplexityModels.length > 0) {
      perplexityModels.forEach((model) => {
        console.log(
          `  • ${model.id} - ${model.name || 'No name'} - $${model.pricing?.prompt || 'N/A'}/${model.pricing?.completion || 'N/A'}`
        );
      });
      ...
      const allModels = models.filter((model) => model.name?.toLowerCase().includes('perplexity'));

      if (allModels.length > 0) {
        console.log(`📊 Found ${allModels.length} models with "perplexity" in name:`);
        allModels.forEach((model) => {
          console.log(
            `  • ${model.id} - ${model.name} - $${model.pricing?.prompt || 'N/A'}/${model.pricing?.completion || 'N/A'}`
          );
        });
      } else {
