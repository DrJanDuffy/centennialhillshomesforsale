...
async function getRandomFeaturedImage(propertyType) {
  try {
...
function loadPropertyTypeImages(propertyType, containerId) {
  ...
  fetchRealEstateImages(query, 6)
    .then(images => displayRealEstateImages(images, containerId));
}
...
function createPropertyGallerySlider(propertyId, imageCount = 5) {
  ...
  fetchRealEstateImages(query, imageCount)
    .then(images => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetchRealEstateImages('real estate home', 12)
    .then(images => displayRealEstateImages(images, 'homepage-gallery'));
