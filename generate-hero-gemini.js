...
/**
 * Generate Hero Image using Google Gemini 2.5 Flash via OpenRouter
 * This script will generate a luxury real estate hero image using Gemini's image generation
 */
...
if (!OPENROUTER_API_KEY || OPENROUTER_API_KEY === 'your-openrouter-api-key-here') {
  ...
  console.log('   OPENROUTER_API_KEY=your-actual-openrouter-api-key');
  console.log('\n🔗 Get your API key from: https://openrouter.ai/keys');
  process.exit(1);
}

async function generateHeroImage() {
  console.log('🎨 Generating luxury hero image with Google Gemini 2.5 Flash...\n');
...
- A professional, aspirational aesthetic, capturing the essence of luxury living in the Nevada desert, with a sense of exclusivity and sophistication.`;
  ...
  try {
    console.log('🚀 Sending request to OpenRouter with Gemini 2.5 Flash...');
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: `Generate a high-quality real estate hero image with this description: ${prompt}. Please provide the image as a base64 encoded string or direct URL.`
              }
            ]
          }
        ],
    ...
    if (data.choices && data.choices[0] && data.choices[0].message) {
      ...
      if (typeof messageContent === 'string') {
        ...
        if (urlMatch) {
          ...
          console.log('⚠️  No image found in response. Gemini may not support direct image generation.');
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        console.log('✨ Enhanced prompt generated by Gemini:');
        console.log('📝', enhancedPrompt);
        
        // Save the enhanced prompt to a file
        fs.writeFileSync('enhanced-hero-prompt.txt', enhancedPrompt);
        console.log('💾 Enhanced prompt saved to: enhanced-hero-prompt.txt');
